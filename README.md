...
Untrusted input is a call; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a call
- **Closed type:** `type Decision = GoTo(Floor) | HoldCar` where `Floor` is closed
- **Sink / capability:** `grant dispatchCar` → `privileged { dispatchCar(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/elevator-dispatch/demo.sh
```
...
- `elevator-dispatch_safe.eide` — the correct design.
- `elevator-dispatch_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
